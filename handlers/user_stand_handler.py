...
例如：/设置替身 AABCDE 白金之星"""
        ...
        # 生成替身面板URL
        image_url = self.api_service.get_image_url(name=display_name, ability=stand_data.abilities)
        
        # 将数字能力值转换回字母显示
        ability_letters = AbilityUtils.convert_abilities_to_letters(stand_data.abilities)
    ...
    async def handle_view_stand(self, event: AstrMessageEvent):
        ...
        for msg_component in messages:
            if isinstance(msg_component, Comp.At):
                ...
                target_user_name = getattr(msg_component, 'name', None) or f"用户{target_user_id}"
                break
