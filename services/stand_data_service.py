...
from ..models.stand_models import StandData, AwakenRecord
...
class StandDataService:
    ...
    def save_user_stand(self, user_id: str, abilities: str, name: Optional[str] = None) -> None:
        """
        保存用户的替身数据到持久化存储
        ...
        stand_data = StandData(
            ...
            abilities=abilities,
            name=name,
            created_at=datetime.datetime.now(self.timezone).strftime("%Y-%m-%d %H:%M:%S")
        )
    ...
    def save_awaken_record(self, user_id: str) -> None:
        ...
        current_record["last_awaken_time"] = datetime.datetime.now(self.timezone).strftime("%Y-%m-%d %H:%M:%S")
        
        awaken_records[user_id][today] = current_record
        
        sp.put(self.AWAKEN_RECORDS_KEY, awaken_records)
    
    def check_awaken_limit(self, user_id: str, daily_limit: int = 1) -> Tuple[bool, str]:
        """
        检查用户今日觉醒次数限制
        ...
        if today_count >= daily_limit:
            ...
            tomorrow = (datetime.datetime.now(self.timezone) + datetime.timedelta(days=1)).strftime("%Y-%m-%d")
            return False, f"❌ 今日觉醒次数已用完！\n\n你今天已经重新觉醒过了（{last_awaken_time}）\n每天只能重新觉醒 {daily_limit} 次，请明天（{tomorrow}）再来尝试！"
